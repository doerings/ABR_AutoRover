...
public class Main_activity extends Activity implements IOIOLooperProvider, SensorEventListener        // implements IOIOLooperProvider: from IOIOActivity
{
	...
	private ToggleButton btnLeftCircle;
	private ToggleButton btnRightCircle;
	...
	public void onCreate(Bundle savedInstanceState) 
	{
		...
		btnLeftCircle = (ToggleButton) findViewById(R.id.buttonLeftCircle);
		btnRightCircle = (ToggleButton) findViewById(R.id.buttonRightCircle);
	...
	public final void onSensorChanged(SensorEvent event) {
		if(m_ioio_thread != null){
			...
			if (btnStartStop.isChecked()) {
				if (btnLeftCircle.isChecked() || btnRightCircle.isChecked()) {
					m_ioio_thread.move(0.0f, 0.0f, false, false);
				}
				else {
					if (m_ioio_thread.getIrLeftReading() < .05 && m_ioio_thread.getIrCenterReading() < .05 && m_ioio_thread.getIrRightReading() < .05) {
						m_ioio_thread.move(0.5f, 0.5f, true, true);
					}
					else {
						if (m_ioio_thread.getIrLeftReading() > m_ioio_thread.getIrCenterReading() && m_ioio_thread.getIrLeftReading() > m_ioio_thread.getIrRightReading()) {
							m_ioio_thread.move(1f, 0f, true, true);
						}
						if (m_ioio_thread.getIrCenterReading() > m_ioio_thread.getIrLeftReading() && m_ioio_thread.getIrCenterReading() > m_ioio_thread.getIrRightReading()) {
							if (m_ioio_thread.getIrCenterReading() < .5) {
								m_ioio_thread.move(0.5f, 0.5f, true, true);
							}
							else {
								if (m_ioio_thread.getIrLeftReading() > m_ioio_thread.getIrRightReading()) {
									m_ioio_thread.move(0f, 0.75f, false, false);
								}
								else {
									m_ioio_thread.move(0.75f, 0f, false, false);
								}
							}
						}
						if (m_ioio_thread.getIrRightReading() > m_ioio_thread.getIrLeftReading() && m_ioio_thread.getIrRightReading() > m_ioio_thread.getIrCenterReading()) {
							m_ioio_thread.move(0f, 1f, true, true);
						}
					}
				}
			}
			else if (btnLeftCircle.isChecked()) {
				if (btnRightCircle.isChecked() || btnStartStop.isChecked()) {
					m_ioio_thread.move(0.0f, 0.0f, false, false);
				}
				else {
					if (m_ioio_thread.getIrLeftReading() < 1 && m_ioio_thread.getIrCenterReading() < 1 && m_ioio_thread.getIrRightReading() < 1) {
			...
			else if (btnRightCircle.isChecked()) {
				if (btnStartStop.isChecked() || btnLeftCircle.isChecked()) {
					m_ioio_thread.move(0.0f,0.0f,false,false);
				}
				else {
					if (m_ioio_thread.getIrLeftReading() < 1 && m_ioio_thread.getIrCenterReading() < 1 && m_ioio_thread.getIrRightReading() < 1) {
